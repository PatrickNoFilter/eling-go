...
- Project Rules (D1, DeepCode heist): `internal/layers/rules_ingest.go` probes AGENTS.md → DEEPCODE.md → CLAUDE.md → .cursor/rules/*.mdc (first match wins), caps at 4 KiB/40 lines, and loads into `Agent.projectRules` at boot (`cfg.Agent.ProjectRules`, default on). `buildMessages()` injects it as a system message after the budget calculation; `eling rules show [--project-dir <dir>]` prints it read-only. Immutable after `New()` → read under `a.mu.RLock()`, same discipline as `a.learnings`
